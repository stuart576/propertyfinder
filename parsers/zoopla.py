"""Parser for Zoopla property alert emails."""
import re
...
class ZooplaParser(BaseParser):
    ...
    def parse(self, html: str, text: str = "") -> list[dict]:
        ...
        # Zoopla emails contain links to zoopla.co.uk/for-sale/details/...
        links = soup.find_all("a", href=re.compile(r"zoopla\.co\.uk.*(?:details|for-sale)", re.IGNORECASE))
        ...
        for link in links:
            ...
            # Navigate up to find property card container
            container = link
            for _ in range(8):
                if container.parent:
    ...
    def _parse_text_fallback(self, text: str) -> list[dict]:
        ...
        for chunk in chunks:
            ...
            if not price:
                continue
            url_match = re.search(r"(https?://[^\s]+zoopla[^\s]+)", chunk)
